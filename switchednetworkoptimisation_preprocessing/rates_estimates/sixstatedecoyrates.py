...
class SixStateDecoyNetwork(Network):
    ...
    def get_single_photon_gain(self):
        ...
        as deviations from standard is detected by decoys: from Lo et al 2005b. For QD it is Y! * p(1)
    ...
    def get_gain_mu(self, mu):
        """
        gets total gain Q_(mu) for a given mu used as decoys: Lo et al 2005b
    ...
    def get_qber_mu(self, mu):
        """
        gets total QBER E_(mu) for a given mu used as decoys: Lo et al 2005b
        :param mu: mean photon number of the decoy state: float
    ...
    def get_gain_qd(self, purity):
        """
        gets total gain Q_(mu) for a given purity used as decoys:
        :param mu: mean photon number of the decoy state: float
        :return: The gain: float
        """
    ...
    def get_qber_qd(self, purity):
        """
        gets total QBER E_(mu) for a given purity used as decoys
        :param mu: mean photon number of the decoy state: float
        :return: The quantum bit error rate: float
        """
        if isinstance(self.source, QDsource):
    ...
    def fit_errors_spdc(self, mu, edet):
        ...
        :param mu: mean photon number for this decoy state: float
        :param edet: The detector error from calculation: float
        :return: The calculation of Q_(mu)E_(mu) using mu and edet as inputs on the LHS of equation: float
        """
    ...
    def get_error_estimates(self):
        ...
        if isinstance(self.source, SPDCSource):
            ...
            mu = [0.0, 0.01,0.05, 0.015, 0.017, 0.019, 0.2]
    ...
    def get_rate(self):
        ...
        if q1 < 0.000000000000001:
            ...
            if isinstance(self.source, SPDCSource):
                e1 = (popt[0]*(1-(1-probofdetection))+ 1/2 * pdark) / probofdetection
            else:
                e1 = popt[0]
            # get gain and QBER
            gain = self.get_gain()
            qber = self.get_qber()
            # calculation R = q1/2[1-H_2(e1)] - Q_mu/2f(E_mu)H_2(E_mu) + q1/2 (H(ez+ey) - H(Z|X))
            rawrate = q1 / 2
            ...
            e_x = e1 / 2
            additionalmutualinfo = q1 * (
                        binaryentropy(qber) - conditional_entropy(ex=e_x, ey=e_x, ez=e_x)) / 2
